...
public class RNSendIntentModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void startActivityForResult(int requestCode, String url, ReadableMap data, Promise promise) {
      Activity currentActivity = getCurrentActivity();

      if (currentActivity == null) {
        promise.reject('fail_activity_not_exists', "Activity doesn't exist");
        return;
      }
      ...
      try {
        ...
        mPromise.put(requestCode, promise);
      } catch (Exception e) {
        mPromise.reject('fail', e);
        mPromise = null;
      }
  }
